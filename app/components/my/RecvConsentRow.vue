<template>
  ...
  <div>
    ...
    <div class="grid grid-cols-2 gap-x-3 gap-y-2 rounded-lg border bg-[#f9fafb] px-3 py-3 text-[0.85rem] sm:grid-cols-4" :class="requiredOk ? 'border-[#e5e7eb]' : 'border-[#fca5a5]'">
    ...
    <p class="m-0 mt-1 text-[0.72rem]" :class="requiredOk ? 'text-gray-400' : 'text-red-500'">휴대폰 또는 이메일 중 1개 이상 필수입니다.</p>
    ...
    <!-- ③ 선택 광고 -->
    <span class="mb-1 mt-3 block text-[0.78rem] text-gray-500">수신 동의 <span class="text-gray-400">(선택)</span> 광고</span>
    <div class="grid grid-cols-3 gap-x-3 gap-y-2 rounded-lg border border-[#e5e7eb] bg-[#f9fafb] px-3 py-3 text-[0.85rem]">
