  ...
  <div class="product__wrapper mb-60 group">
    ...
    <div class="product__content relative">
      <div class="product__content-inner">
        ...
        <div v-if="hasRating" class="mb-2 flex items-center gap-1 text-[12px]" :title="`평점 ${Number(item.rating || 0).toFixed(1)} (${item.reviewCnt ?? 0}개 상품평)`">
...
const props = defineProps<{
   ...
   *  목록 화면(shop.vue)에서만 넘겨준다 — 슬라이더 등 다른 곳에서 쓸 땐 비워두면 배지가 아예 안 보인다. */
  rowNo?: number;
  total?: number;
}>();
