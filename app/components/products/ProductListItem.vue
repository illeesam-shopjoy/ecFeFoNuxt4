        ...
        <div class="product__content relative">
          <div class="product__content-inner list">
            ...
            <!-- 2026-09-13(요청사항: "평가도 보여야 하고") — item.rating은 목록 조회에서는
                 상품평 요약을 따로 안 불러와 항상 0이지만(상세 페이지에서만 채워짐), 별점 UI
                 구조 자체는 데모와 맞춰 노출해둔다. -->
            <div v-if="hasRating" class="mb-10 flex items-center gap-1 text-[13px]" :title="`평점 ${Number(item.rating || 0).toFixed(1)} (${item.reviewCnt ?? 0}개 상품평)`">
