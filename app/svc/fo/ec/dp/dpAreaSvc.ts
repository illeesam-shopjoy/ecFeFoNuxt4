...
export const dpAreaSvc = {
  ...
  getActiveItems: async (areaCd: string): Promise<DpPanelItemType[]> => (await axiosCsr.get<DpPanelItemType[]>(`/fo/ec/dp/area/${encodeURIComponent(areaCd)}`)).data ?? [],

  /** 영역의 첫 위젯 config(JSON). 미등록/조회실패/파싱실패는 null → 호출부가 기본값 폴백 */
  async getFirstWidgetConfig<T>(areaCd: string): Promise<T | null> {
    return parseFirstWidgetConfig<T>(await dpAreaSvc.getActiveItems(areaCd).catch(() => null));
  },
};
