...
export const syCodeSvc = {
  ...
  getCodes: (): Promise<SyCodeType[]> => csrList<SyCodeType>("/co/sy/code").then(mapCodes),

  /** GET /co/sy/code/groups — 지정한 코드그룹만(여러 그룹 한 번에). 화면이 쓰는 그룹만 받아 누적 적재할 때 쓴다 */
  getByGroups: (codeGrps: string[]): Promise<SyCodeType[]> => csrList<SyCodeType>("/co/sy/code/groups", { params: { codeGrps } }).then(mapCodes),
};
