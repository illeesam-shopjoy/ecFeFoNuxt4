 ...
 * 응답은 PdProdType 모양을 유지하되 무거운 필드만 비운다(화면이 배열 필드를 그대로 읽으므로 undefined 로 두지 않는다).
 */
export default defineEventHandler(async (event) => {
  ...
  Object.assign(out, { contentHtml: "", relatedImages: [], prodOpt1List: [], prodOpt2List: [], prodSkus: [], reviews: [] });
