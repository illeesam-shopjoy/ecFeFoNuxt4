 ...
 * - stale-if-error: 원본(함수→NAS)이 502 등으로 실패해도 하루까지는 직전 캐시본을 대신 내려준다(콜드스타트 직후 첫 호출 실패 완화).
...
export function cdnCache(event: H3Event, seconds: number, swrSeconds = seconds * 5): void {
  setHeader(event, "netlify-cdn-cache-control", `public, max-age=${seconds}, stale-while-revalidate=${swrSeconds}, stale-if-error=86400, durable`);
}
